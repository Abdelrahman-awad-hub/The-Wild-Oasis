...
function CreateCabinForm({ cabinToUpdate, id, onCloseModal }) {

  // know if it update or create session
  ...
  const { register, handleSubmit, reset, getValues, formState } = useForm({
    defaultValues: isUpdateSession ? cabinToUpdate : {}
  })
    ...
    <Form onSubmit={handleSubmit(onSubmit, onError)} type="modal">
      ...
      <FormRow label='Regular price' error={errors?.regularPrice?.message}>
        <Input type="number" id="regularPrice" disabled={isWorking} {...register('regularPrice', {
          ...
          min: {
            value: 1,
            message: 'Regular price should be at lest $1'
          }
        })} />
      </FormRow>
